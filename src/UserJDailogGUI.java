...
public class UserJDailogGUI extends JDialog implements ActionListener{
    ...
    JPanel pc=new JPanel(new GridLayout(3,1));
    JPanel ps=new JPanel();

    JLabel lable_Email=new JLabel("이메일");
    JLabel lable_Name=new JLabel("이름");
    JLabel label_Password=new JLabel("패스워");
    ...
    public UserJDailogGUI(MenuJTabaleExam me, String index){
        ...
        if(index.equals("가입")){
            ...
            email.setText( me.jt.getValueAt(row, 0).toString() );
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(btnLabel.equals("가입")){
            ...
            if(email.getText().trim().equals("")){
                messageBox(this,"email을 입력해주세요.");
                email.requestFocus();//포커스이동
            }else{

                if(  dao.getIdByCheck(email.getText()) ){ //중복아니다.(사용가능)
                    messageBox(this, email.getText()+"는 사용가능합니다.");
                }else{ //중복이다.
                    messageBox(this,email.getText()+"는 중복입니다.");

                    email.setText("");//text박스지우기
                    email.requestFocus();//커서놓기
                }
